    ...
    switch (val->type & VALUE_MODE_MASK) {
      ...
      case MODE_BITSET: {
        ...
        for (uint32_t tableValueIndex = 0; tableValueIndex < tableEntry->value_count && !matched; tableValueIndex++) {
          matched = (tableEntry->values[tableValueIndex] != nullptr) &&
                    (strcasecmp(tableEntry->values[tableValueIndex], eqptr) == 0);
...
static void print_action_config() {
  ...
  for (int i = 0; i < NUM_EVENTS; i++) {
    const int nr_actions = cc_get_num_actions(static_cast<cats_event_e>(i));
    if (nr_actions > 0) {
      cli_printf("\n%s\n", p_event_table->values[i]);
      cli_printf("   Number of Actions: %d\n", nr_actions);
      for (int j = 0; j < nr_actions; j++) {
        cc_get_action(static_cast<cats_event_e>(i), j, &action);
