...
public:
  ...
  void initBar();
  void updateBar(float batteryVoltage, bool usb = false, bool logging = false,
                 bool location = false, bool time = false,
  ...
  void updateKeyboard(char *text, int32_t keyHighlight,
...
private:
  ...
  void addSettingEntry(uint32_t settingIndex, const device_settings_t *setting,
                       bool color = BLACK);
  ...
  const char *eventName[9] = {"Ready", "Liftoff",   "Burnout",  "Apogee",
                              "Main",  "Touchdown", "Custom 1", "Custom 2"};
};