  ...
  if (keyboardActive) {
    ...
    if (okButton.wasPressed() || okButton.pressedFor(500)) {
      if (i == 29) { // shift
        ...
        memcpy((char *)settingsTable[settingSubMenu][settingIndex].dataPtr,
               keyboardString, 8);
        window.initSettings(settingSubMenu);
        configChanged = true;
    ...
    if (settingIndex == -1) {
      ...
      if (settingsTable[settingSubMenu][settingIndex].type == TOGGLE) {
        if (rightButton.wasPressed() &&
            *(bool *)settingsTable[settingSubMenu][settingIndex].dataPtr ==
                false) {
      ...
      if (settingsTable[settingSubMenu][settingIndex].type == STRING) {
        if (okButton.wasPressed()) {
          memcpy(keyboardString,
                 (char *)settingsTable[settingSubMenu][settingIndex].dataPtr,
                 8);

          window.initKeyboard(
              keyboardString,
              settingsTable[settingSubMenu][settingIndex].config.stringLength);
          keyboardActive = true;
        }
      }
    }
  ...
  while (ref->initialized) {
    ...
    if (millis() - barUpdate >= 1000) {
      ...
      if (link2.time.isUpdated()) {
        setTime(link2.time.hour(), link2.time.minute(), link2.time.second(), 0,
                0, 0);
        adjustTime(systemConfig.config.timeZoneOffset * 3600);
        timeValid = true;
      }
      ref->window.updateBar(voltage, digitalRead(21), ref->isLogging,
                            link2.location.isValid(), timeValid, free_memory);
    }
