...
void Window::updateBar(float batteryVoltage, bool usb, bool logging,
                       bool location, bool time, int32_t free_memory) {
...
void Window::updateLive(TelemetryData *data, TelemetryInfo *info,
                        uint32_t index) {
  if (index > 1)
    return;
...
const char *const stateName[] = {"INVALID", "CALIB",  "READY", "THRUST",
                                 "COAST",   "DROGUE", "MAIN",  "DOWN"};

const char *const errorName[] = {"No Config", "Log Full", "Filter Error",
                                 "Overheating", "Continuity Error"};

void Window::updateLiveData(TelemetryData *data, uint32_t index,
                            uint32_t color) {
...
void Window::updateLiveInfo(TelemetryInfo *info, uint32_t index,
                            uint32_t color) {
...
void Window::addSettingEntry(uint32_t settingIndex,
                             const device_settings_t *setting, bool color) {
  ...
  if (setting->type == TOGGLE) {
    ...
    drawCentreString(lookup_tables[setting->config.lookup]
                         .values[static_cast<uint16_t>(data)],
                     305, y);
...
void Window::initKeyboard(char *text, uint32_t maxLength) {
  ...
  for (int i = 0; i < 38; i++) {
    if (i == oldKey) {
      ...
      if (!upperCase && i > 9)
        display.drawChar(keybXY[i][0], keybXY[i][1], keybChar[i] + 32, BLACK,
                         WHITE, 2);
      else
        display.drawChar(keybXY[i][0], keybXY[i][1], keybChar[i], BLACK, WHITE,
                         2);
    }
  }
  if (oldKey != 29)
    display.drawBitmap(keybXY[29][0] - 4, keybXY[29][1] - 1, shift_keyboard, 16,
                       16, BLACK);
  if (oldKey != 37)
    display.drawBitmap(keybXY[37][0] - 4, keybXY[37][1] - 1, enter_keyboard, 16,
                       16, BLACK);
  if (oldKey != -1)
...
void Window::highlightKeyboardKey(int32_t key, bool color) {
  ...
  if (key == 29) {
    ...
    if (!upperCase && key > 9)
      display.drawChar(keybXY[key][0], keybXY[key][1], keybChar[key] + 32,
                       !color, color, 2);
    else
      display.drawChar(keybXY[key][0], keybXY[key][1], keybChar[key], !color,
                       color, 2);
  }
}
