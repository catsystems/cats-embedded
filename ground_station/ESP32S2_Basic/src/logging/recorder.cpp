...
void Recorder::recordTask(void *pvParameter) {
  ...
  while (ref->initialized) {
    if (xQueueReceive(ref->queue, &element, portMAX_DELAY) == pdPASS) {
      ...
      snprintf(line, 128, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", element.timestamp,
               element.state, element.errors, element.lat, element.lon,
               element.altitude, element.velocity, element.voltage,
               (bool)(element.pyro_continuity & 0x01),
               (bool)(element.pyro_continuity & 0x02));
