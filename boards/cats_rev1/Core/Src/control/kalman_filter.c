...
cats_status_e kalman_update_eliminated(kalman_filter_t *filter, state_estimation_data_t *data, sensor_elimination_t *elimination){
	...
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 3; j++) {
			for (int k = 0; k < 3; k++) {
				placeholder_mat[i][j] += filter->H_eliminated[i][k] * filter->P_hat[k][j];
			}
		}
	}
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 2; j++) {
			for (int k = 0; k < 3; k++) {
				placeholder_mat3[i][j] += filter->P_hat[i][k] * filter->H_eliminated_T[k][j];
			}
		}
	}
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 2; j++) {
			for (int k = 0; k < 2; k++) {
				filter->K_eliminated[i][j] += placeholder_mat3[i][k] * placeholder_mat4[k][j];
			}
		}
	}
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			for (int k = 0; k < 2; k++) {
				placeholder_mat5[i][j] += filter->K_eliminated[i][k] * filter->H_eliminated[k][j];
			}
			if (i == j) {
