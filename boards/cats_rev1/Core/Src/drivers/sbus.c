...
uint8_t sbus_input_buffer[25];
uint8_t receiver_data_raw[25];
...
void map_channels(uint8_t* _payload) {
  receiver_data.ch[0] = (uint16_t)((_payload[0] | _payload[1] << 8) & 0x07FF);
  receiver_data.ch[1] =
      (uint16_t)((_payload[1] >> 3 | _payload[2] << 5) & 0x07FF);
  receiver_data.ch[2] = (uint16_t)(
  ...
  receiver_data.ch[8] = (uint16_t)((_payload[11] | _payload[12] << 8) & 0x07FF);
  receiver_data.ch[9] =
      (uint16_t)((_payload[12] >> 3 | _payload[13] << 5) & 0x07FF);
  receiver_data.ch[10] = (uint16_t)(
  ...
  for (int i = 0; i < 16; i++) {
    if (receiver_data.ch[i] != 0)
      receiver_data.ch[i] = linear_fit(receiver_data.ch[i]);
  }
  receiver_data.failsafe = (_payload[22] & 0x08) ? 1 : 0;
  receiver_data.framelost = (_payload[22] & 0x04) ? 1 : 0;
}
