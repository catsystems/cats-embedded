...
_Noreturn void task_state_est(__attribute__((unused)) void *argument) {
    ...
    while (1) {
        ...
        filtered_data_info_t filtered_data_info = {.filtered_acceleration =
    ...
    switch (calibration->axis) {
        case 0:
            ...
            state_data->acceleration[0] =
                    (float) (global_imu[0].acc_x) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            state_data->acceleration[1] =
                    (float) (global_imu[1].acc_x) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            state_data->acceleration[2] =
                    (float) (global_imu[2].acc_x) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            break;
        case 1:
            /* Choose Y Axis */
            state_data->acceleration[0] =
                    (float) (global_imu[0].acc_y) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            state_data->acceleration[1] =
                    (float) (global_imu[1].acc_y) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            state_data->acceleration[2] =
                    (float) (global_imu[2].acc_y) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            break;
        case 2:
            /* Choose Z Axis */
            state_data->acceleration[0] =
                    (float) (global_imu[0].acc_z) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            state_data->acceleration[1] =
                    (float) (global_imu[1].acc_z) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            state_data->acceleration[2] =
                    (float) (global_imu[2].acc_z) / (1024) * GRAVITY / calibration->angle - GRAVITY;
            break;
        default:
            break;
    }
...
static void median_filter(median_filter_t *filter_data, state_estimation_data_t *state_data) {
    ...
    state_data->acceleration[0] = median(&filter_data->acc_data[0][0], MEDIAN_FILTER_SIZE);
    state_data->acceleration[1] = median(&filter_data->acc_data[1][0], MEDIAN_FILTER_SIZE);
    state_data->acceleration[2] = median(&filter_data->acc_data[2][0], MEDIAN_FILTER_SIZE);
    state_data->calculated_AGL[0] = median(&filter_data->height_AGL_data[0][0], MEDIAN_FILTER_SIZE);
    state_data->calculated_AGL[1] = median(&filter_data->height_AGL_data[1][0], MEDIAN_FILTER_SIZE);
    state_data->calculated_AGL[2] = median(&filter_data->height_AGL_data[2][0], MEDIAN_FILTER_SIZE);
}
