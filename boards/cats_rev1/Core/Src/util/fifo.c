...
uint8_t fifo_read(fifo_t *fifo) {
  if ((fifo->used == 0) || fifo->mutex) return 0;
  fifo->mutex = true;
...
bool fifo_write(fifo_t *fifo, uint8_t data) {
  if (fifo->used >= fifo->size || fifo->mutex) return false;
...
bool fifo_read_bytes(fifo_t *fifo, uint8_t* data, uint32_t count){
  if((fifo->used < count) || fifo->mutex) return false;
  fifo->mutex = true;
...
bool fifo_write_bytes(fifo_t *fifo, uint8_t* data, uint32_t count){
  ...
  if((fifo->size-fifo->used) < count || fifo->mutex) return false;
  fifo->mutex = true;
