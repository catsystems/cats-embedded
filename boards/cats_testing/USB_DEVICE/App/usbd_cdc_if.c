...
static int8_t CDC_Receive_FS(uint8_t *Buf, uint32_t *Len) {
  ...
  if (buf_length != 0) {
    memcpy(usb_receive_buffer, Buf, buf_length < APP_RX_DATA_SIZE ? buf_length : APP_RX_DATA_SIZE);
    usb_receive_buffer[buf_length < APP_RX_DATA_SIZE ? buf_length : buf_length - 1] = 0;
