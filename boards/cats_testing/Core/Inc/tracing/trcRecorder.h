...
#define vTracePrintF(chn, fmt, ...) \
  (void)(chn),                      \
      (void)(fmt) /* Comma operator is used to avoid "unused variable" compiler warnings in a single statement */
#endif
...
#define vTraceVPrintF(chn, formatStr, vl) \
  (void)(chn), (void)(formatStr),         \
      (void)(vl) /* Comma operator is used to avoid "unused variable" compiler warnings in a single statement */
#endif
...
#define vTracePrint(chn, str) \
  (void)(chn),                \
      (void)(str) /* Comma operator is used to avoid "unused variable" compiler warnings in a single statement */
#endif
...
#define xTraceRegisterString(x)                                                                                     \
  ((void)(x), (traceString)0) /* Comma operator in parenthesis is used to avoid "unused variable" compiler warnings \
                                 and return 0 in a single statement */
#endif
...
#else
#define vTraceSetRecorderDataBuffer(pRecorderData)    /* If not CUSTOM, pRecorderData will be an undefined symbol (same \
                                                         as in TRC_ALLOC_CUSTOM_BUFFER), so no (void) here */
#endif
...
#define TRC_ALLOC_CUSTOM_BUFFER(bufname) \
  char bufname[TRC_CFG_RTT_BUFFER_SIZE_UP]; /* Not static in this case, since declared in user code */
#else
#define TRC_ALLOC_CUSTOM_BUFFER(bufname) \
  char bufname[(TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) * (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE)];
#endif
#endif
#else
#define TRC_ALLOC_CUSTOM_BUFFER(bufname)    /* If not CUSTOM, bufname will be an undefined symbol (same as in \
...
#define trcKERNEL_HOOKS_OBJECT_CREATE(SERVICE, CLASS, pxObject)                                                 \
  ...
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                              \
    if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask)                                           \
      prvTraceStoreKernelCall(SERVICE, TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),                           \
...
#define trcKERNEL_HOOKS_OBJECT_DELETE(SERVICE, SERVICE_NAME, SERVICE_PROP, CLASS, pxObject)         \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                  \
    if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask)                               \
      prvTraceStoreKernelCall(SERVICE, TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),               \
                              TRACE_GET_OBJECT_NUMBER(CLASS, pxObject));                            \
  prvTraceStoreObjectNameOnCloseEvent(SERVICE_NAME, TRACE_GET_OBJECT_NUMBER(CLASS, pxObject),       \
                                      TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject));               \
  prvTraceStoreObjectPropertiesOnCloseEvent(SERVICE_PROP, TRACE_GET_OBJECT_NUMBER(CLASS, pxObject), \
                                            TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject));         \
...
#define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(SERVICE, CLASS, pxObject, param)             \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)             \
    if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask)                          \
      prvTraceStoreKernelCallWithParam(SERVICE, TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject), \
                                       TRACE_GET_OBJECT_NUMBER(CLASS, pxObject), (uint32_t)param);

/* This macro will create a call to a kernel service with a certain result, with a null object and other value as
 * parameter */
...
#define trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, pxObject, STATE)                                                  \
  prvTraceSetObjectState(TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject), TRACE_GET_OBJECT_NUMBER(CLASS, pxObject), \
                         (uint8_t)STATE);
...
#define trcKERNEL_HOOKS_MOVED_TASK_TO_READY_STATE(pxTCB) \
  if (TRACE_GET_OBJECT_FILTER(TASK, pxTCB) & CurrentFilterMask) prvTraceStoreTaskReady(TRACE_GET_TASK_NUMBER(pxTCB));
...
#define trcKERNEL_HOOKS_NEW_TIME(SERVICE, xValue) prvTraceStoreKernelCallWithNumericParamOnly(SERVICE, xValue);
...
#define trcKERNEL_HOOKS_TASK_SWITCH(pxTCB) \
  if (TRACE_GET_OBJECT_FILTER(TASK, pxTCB) & CurrentFilterMask) prvTraceStoreTaskswitch(TRACE_GET_TASK_NUMBER(pxTCB));
...
#define trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(SERVICE, pxTCB, uxNewPriority)                                        \
  if (TRACE_GET_OBJECT_FILTER(TASK, pxTCB) & CurrentFilterMask) {                                                  \
    prvTraceStoreKernelCallWithParam(SERVICE, TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB),                      \
                                     prvTraceGetPriorityProperty(TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB))); \
...
#define vTraceSetISRProperties(handle, name, priority) \
  (void)(handle), (void)(name),                        \
      (void)(priority) /* Comma operator is used to avoid "unused variable" compiler warnings in a single statement */
...
#define xTraceSetISRProperties(name, priority)                                                                         \
  ((void)(name), (void)(priority), (traceHandle)0) /* Comma operator in parenthesis is used to avoid "unused variable" \
...
#define xTraceRegisterChannelFormat(eventLabel, formatStr)                                                       \
  ((void)(eventLabel), (void)(formatStr), 0) /* Comma operator in parenthesis is used to avoid "unused variable" \
                                                compiler warnings and return 0 in a single statement */
#undef vTraceUBData
...
#define TRC_STREAM_PORT_ALLOCATE_DYNAMIC_EVENT(_type, _ptrData, _size) \
  TRC_STREAM_PORT_ALLOCATE_EVENT(_type, _ptrData, _size) /* We do the same thing as for non-dynamic event sizes */
#else
#define TRC_STREAM_PORT_ALLOCATE_DYNAMIC_EVENT(_type, _ptrData, _size) \
...
void prvTraceStoreEvent3(uint16_t eventID, uint32_t param1, uint32_t param2, uint32_t param3);
...
#define xTraceSetISRProperties(a, b)                                                                                  \
  ((void)(a), (void)(b), (traceHandle)0) /* Comma operator in parenthesis is used to avoid "unused variable" compiler \
                                            warnings and return 0 in a single statement */
#define vTraceStoreKernelObjectName(a, b) \
  (void)(a), (void)(b) /* Comma operator is used to avoid "unused variable" compiler warnings in a single statement */
#define xTraceRegisterChannelFormat(eventLabel, formatStr)                                                       \
  ((void)(eventLabel), (void)(formatStr), 0) /* Comma operator in parenthesis is used to avoid "unused variable" \
                                                compiler warnings and return 0 in a single statement */
#define vTraceChannelPrint(label) (void)(label)
...
#define vTraceSetRecorderDataBuffer( \
    pRecorderData) /* No (void) here - ignore parameter since undefined symbol if custom allocation is not used */
#endif
