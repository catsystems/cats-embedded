...
static traceString prvTraceCreateSymbolTableEntry(const char* name, uint8_t crc6, uint8_t len, traceString channel);

static traceString prvTraceLookupSymbolTableEntry(const char* name, uint8_t crc6, uint8_t len, traceString channel);
 ...
 * for snapshot mode.
 ******************************************************************************/
void vTraceSetStopHook(TRACE_STOP_HOOK stopHookFunction) { vTraceStopHookPtr = stopHookFunction; }
...
void vTraceStoreISRBegin(traceHandle handle) {
  ...
  if (RecorderDataPtr->recorderActive && handle_of_last_logged_task) {
    ...
    TRACE_ASSERT(handle <= RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[TRACE_CLASS_ISR],
                 "vTraceStoreISRBegin: Invalid ISR handle (> NISR)", TRC_UNUSED);
...
void vTraceStoreISREnd(int pendingISR) {
  ...
  if (nISRactive > 0) {
    ...
    hnd8 = prvTraceGet8BitHandle(isrstack[nISRactive - 1]); /* isrstack[nISRactive] is the handle of the ISR we're
                                                               currently exiting. isrstack[nISRactive - 1] is the handle
                                                               of the ISR that was executing previously. */
...
/* ISR tracing is turned off */
void prvTraceIncreaseISRActive(void) {
  if (RecorderDataPtr->recorderActive && handle_of_last_logged_task) nISRactive++;
}
      ...
      if (formatStr[formatStrIndex] != '\0') {
        switch (formatStr[formatStrIndex]) {
          ...
          case 'u':
            ...
            break;
          case 's':
            i = writeInt16(buffer, i, xTraceRegisterString((char*)va_arg(vl, char*)));
            break;
...
static void prvTraceClearChannelBuffer(uint32_t count) {
  ...
  if (RecorderDataPtr->userEventBuffer.nextSlotToWrite + count > (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE)) {
    slots = (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE)-RecorderDataPtr->userEventBuffer
                .nextSlotToWrite; /* Number of slots before end of buffer */
    (void)memset(&RecorderDataPtr->userEventBuffer.channelBuffer[RecorderDataPtr->userEventBuffer.nextSlotToWrite], 0,
                 slots);
...
static void prvTraceCopyToDataBuffer(uint32_t* data, uint32_t count) {
  ...
  TRACE_ASSERT(data != NULL, "prvTraceCopyToDataBuffer: data == NULL.", TRC_UNUSED);
...
static void prvTraceUBHelper2(traceUBChannel channel, uint32_t* data, uint32_t noOfSlots) {
  ...
  TRACE_ASSERT((TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE) >= noOfSlots,
               "prvTraceUBHelper2: TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE is too small to handle this event.",
               TRC_UNUSED);
  ...
  RecorderDataPtr->userEventBuffer.nextSlotToWrite =
      ...
      (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE); /* Make sure we never end up outside the buffer */
...
traceUBChannel xTraceRegisterUBChannel(traceString channel, traceString formatStr) {
  ...
  for (i = 1; i <= (TRC_CFG_UB_CHANNELS);
       i++) /* Size of the channels buffer is TRC_CFG_UB_CHANNELS + 1. Index 0 is unused. */
  {
...
using a memset to zero).*/
        ...
        (void)memcpy(&RecorderDataPtr->eventData[RecorderDataPtr->nextFreeIndex * 4], tempDataBuffer, noOfSlots * 4);
...
void prvTraceStoreKernelCall(uint32_t ecode, traceObjectClass objectClass, uint32_t objectNumber) {
  ...
  TRACE_ASSERT(objectNumber <= RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[objectClass],
               "prvTraceStoreKernelCall: Invalid value for objectNumber", TRC_UNUSED);
 ...
 ******************************************************************************/
#if (TRC_CFG_SCHEDULING_ONLY == 0)
void prvTraceStoreKernelCallWithParam(uint32_t evtcode, traceObjectClass objectClass, uint32_t objectNumber,
                                      uint32_t param) {
  ...
  TRACE_ASSERT(objectClass < TRACE_NCLASSES, "prvTraceStoreKernelCallWithParam: objectClass >= TRACE_NCLASSES",
               TRC_UNUSED);
  TRACE_ASSERT(objectNumber <= RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[objectClass],
               "prvTraceStoreKernelCallWithParam: Invalid value for objectNumber", TRC_UNUSED);
...
void prvTraceStoreTaskswitch(traceHandle task_handle) {
  ...
  TRACE_ASSERT(task_handle <= (TRC_CFG_NTASK), "prvTraceStoreTaskswitch: Invalid value for task_handle", TRC_UNUSED);
  ...
  if ((task_handle != handle_of_last_logged_task) && (RecorderDataPtr->recorderActive)) {
    ...
    if (ts != NULL) {
      if (prvTraceGetObjectState(TRACE_CLASS_TASK, handle_of_last_logged_task) == TASK_STATE_INSTANCE_ACTIVE) {
      ...
      prvTraceSetObjectState(TRACE_CLASS_TASK, handle_of_last_logged_task, TASK_STATE_INSTANCE_ACTIVE);
...
void prvTraceStoreObjectNameOnCloseEvent(uint8_t evtcode, traceHandle handle, traceObjectClass objectclass) {
  ...
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceStoreObjectNameOnCloseEvent: objectclass >= TRACE_NCLASSES",
               TRC_UNUSED);
...
void prvTraceStoreObjectPropertiesOnCloseEvent(uint8_t evtcode, traceHandle handle, traceObjectClass objectclass) {
  ...
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceStoreObjectPropertiesOnCloseEvent: objectclass >= TRACE_NCLASSES",
               TRC_UNUSED);
  TRACE_ASSERT(handle <= RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[objectclass],
...
void prvTraceSetPriorityProperty(uint8_t objectclass, traceHandle id, uint8_t value) {
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceSetPriorityProperty: objectclass >= TRACE_NCLASSES", TRC_UNUSED);
...
uint8_t prvTraceGetPriorityProperty(uint8_t objectclass, traceHandle id) {
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceGetPriorityProperty: objectclass >= TRACE_NCLASSES", 0);
  TRACE_ASSERT(id <= RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[objectclass],
...
void prvTraceSetObjectState(uint8_t objectclass, traceHandle id, uint8_t value) {
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceSetObjectState: objectclass >= TRACE_NCLASSES", TRC_UNUSED);
  TRACE_ASSERT(id <= RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[objectclass],
...
uint8_t prvTraceGetObjectState(uint8_t objectclass, traceHandle id) {
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceGetObjectState: objectclass >= TRACE_NCLASSES", 0);
  TRACE_ASSERT(id <= RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[objectclass],
...
traceHandle prvTraceGetObjectHandle(traceObjectClass objectclass) {
  ...
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceGetObjectHandle: Invalid value for objectclass", (traceHandle)0);
  ...
  if (objectHandleStacks.objectHandles[indexOfHandle] == 0) {
    ...
    objectHandleStacks.objectHandles[indexOfHandle] =
        (traceHandle)(1 + indexOfHandle - objectHandleStacks.lowestIndexOfClass[objectclass]);
  }
  ...
  if (objectHandleStacks.indexOfNextAvailableHandle[objectclass] >
    ...
    hndCount =
        objectHandleStacks.indexOfNextAvailableHandle[objectclass] - objectHandleStacks.lowestIndexOfClass[objectclass];

    if (hndCount > objectHandleStacks.handleCountWaterMarksOfClass[objectclass]) {
      objectHandleStacks.handleCountWaterMarksOfClass[objectclass] = (traceHandle)hndCount;
    }
  }
  trcCRITICAL_SECTION_END();
...
void prvTraceFreeObjectHandle(traceObjectClass objectclass, traceHandle handle) {
  ...
  TRACE_ASSERT(objectclass < TRACE_NCLASSES, "prvTraceFreeObjectHandle: Invalid value for objectclass", TRC_UNUSED);
...
void prvTraceSetObjectName(traceObjectClass objectclass, traceHandle handle, const char* name) {
  ...
  if (handle > RecorderDataPtr->ObjectPropertyTable.NumberOfObjectsPerClass[objectclass]) {
    ...
    if (traceErrorMessage == NULL) {
      prvStrncpy((char*)&(RecorderDataPtr->ObjectPropertyTable.objbytes[idx]), name,
...
void vTraceSetFilterMask(uint16_t filterMask) { CurrentFilterMask = filterMask; }

void vTraceSetFilterGroup(uint16_t filterGroup) { CurrentFilterGroup = filterGroup; }
 ...
 * zero-termination
 ******************************************************************************/
traceString prvTraceLookupSymbolTableEntry(const char* name, uint8_t crc6, uint8_t len, traceString chn) {
 ...
 * zero-termination
 ******************************************************************************/
uint16_t prvTraceCreateSymbolTableEntry(const char* name, uint8_t crc6, uint8_t len, traceString channel) {
  uint16_t ret = 0;
  ...
  if (RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + len + 4 >= (TRC_CFG_SYMBOL_TABLE_SIZE)) {
    ...
    prvStrncpy((char*)&(RecorderDataPtr->SymbolTable.symbytes[RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + 4]),
               name, len);
...
#error "TRC_HWTC_TYPE has unexpected value"
  ...
  The scaled timestamp returned from this function is supposed to go from 0 -> 2^32, which in real time would represent
  (0 -> 2^32 * TRC_HWTC_DIVISOR) ticks. */

  /* First we see how long time has passed since the last timestamp call, and we also add the ticks that was lost when
   * we scaled down the last time. */
