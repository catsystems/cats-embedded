...
#define SYMBOL_TABLE_SLOT_SIZE \
  (sizeof(uint32_t) + (((TRC_CFG_SYMBOL_MAX_LENGTH) + (sizeof(uint32_t) - 1)) / sizeof(uint32_t)) * sizeof(uint32_t))
...
uint32_t TotalBytesRemaining_LowWaterMark =
    (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) * (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE);
...
static void prvTraceStoreStringEventHelper(int nArgs, uint16_t eventID, traceString userEvtChannel, int len,
                                           const char* str, va_list vl);

/* Not static to avoid warnings from SysGCC/PPC */
void prvTraceStoreSimpleStringEventHelper(uint16_t eventID, traceString userEvtChannel, const char* str);
 ...
 *****************************************************************************/
void vTraceInstanceFinishedNow(void) { prvTraceStoreEvent0(PSF_EVENT_IFE_DIRECT); }
 ...
 *****************************************************************************/
void vTraceInstanceFinishedNext(void) { prvTraceStoreEvent0(PSF_EVENT_IFE_NEXT); }
...
void vTraceConsoleChannelPrintF(const char* fmt, ...) {
  ...
  if (consoleChannel == NULL) consoleChannel = xTraceRegisterString("Debug Console");
 ...
 * %X - hexadecimal (uppercase). The following width and padding format is supported: "%04X" -> "002A" and "%4X" -> "
 *2A" %x - hexadecimal (lowercase). The following width and padding format is supported: "%04x" -> "002a" and "%4x" -> "
 *2a" %s - string (currently, this must be an earlier stored symbol name)
 *
...
void vTraceSetRecorderDataBuffer(void* pRecorderData) { _TzTraceData = pRecorderData; }
#endif
...
void prvTraceSaveObjectSymbol(void* address, const char* name) {
  ...
  if (firstFreeSymbolTableIndex < SYMBOL_TABLE_BUFFER_SIZE) {
    ...
    symbolTable.SymbolTableBuffer.pSymbolTableBufferUINT32[firstFreeSymbolTableIndex / sizeof(uint32_t)] =
        (uint32_t)address;
      ...
      if (firstFreeSymbolTableIndex > SYMBOL_TABLE_SLOT_SIZE &&
        ...
        lastEntryPtr =
            &symbolTable.SymbolTableBuffer
                 .pSymbolTableBufferUINT32[(firstFreeSymbolTableIndex - SYMBOL_TABLE_SLOT_SIZE) / sizeof(uint32_t)];
...
void prvTraceSaveObjectData(const void* address, uint32_t data) {
  ...
  if (foundSlot < OBJECT_DATA_TABLE_BUFFER_SIZE) {
    /* We access the data table via the union member pObjectDataTableBufferUINT32 to avoid strict-aliasing issues */
    objectDataTable.ObjectDataTableBuffer.pObjectDataTableBufferUINT32[foundSlot / sizeof(uint32_t)] =
        (uint32_t)address;
  ...
  for (i = 0; i < firstFreeObjectDataTableIndex; i += OBJECT_DATA_SLOT_SIZE) {
    ...
    if (*ptr == (uint32_t)address) {
      ...
      if (firstFreeObjectDataTableIndex > OBJECT_DATA_SLOT_SIZE &&
        ...
        /* Another entry is available, get pointer to the last one */
        /* We access the data table via the union member pObjectDataTableBufferUINT32 to avoid strict-aliasing issues */
        lastEntryPtr = &objectDataTable.ObjectDataTableBuffer
                            .pObjectDataTableBufferUINT32[(firstFreeObjectDataTableIndex - OBJECT_DATA_SLOT_SIZE) /
                                                          sizeof(uint32_t)];
...
int prvIsValidCommand(TracealyzerCommandType* cmd) {
  uint16_t checksum =
      (uint16_t)(0xFFFF - (cmd->cmdCode + cmd->param1 + cmd->param2 + cmd->param3 + cmd->param4 + cmd->param5));
...
void prvTraceWarning(int errCode) {
  if (GET_ERROR_WARNING_FLAG(errCode) == 0) {
    /* Will never reach this point more than once per warning type, since we verify if ErrorAndWarningFlags[errCode] has
     * already been set */
...
void prvTraceError(int errCode) {
  if (errorCode == PSF_ERROR_NONE) {
    ...
    prvTraceStoreSimpleStringEventHelper(PSF_EVENT_USER_EVENT, trcWarningChannel,
  ...
  if (pageToTransfer > -1) {
    while (1) /* Keep going until we have transferred all that we intended to */
    {
      if (TRC_STREAM_PORT_WRITE_DATA(
              ...
              (uint32_t)(bytesToTransfer - bytesTransferredTotal), &bytesTransferredNow) == 0) {
        ...
        if (bytesTransferredTotal == bytesToTransfer) {
          /* All bytes have been transferred. Mark the buffer page as "Read Complete" (so it can be written to) and
           * return OK. */
...
void* prvPagedEventBufferGetWritePointer(int sizeOfEvent) {
  ...
  if (PageInfo[currentWritePage].BytesRemaining - sizeOfEvent < 0) {
    ...
    if (TotalBytesRemaining < TotalBytesRemaining_LowWaterMark) TotalBytesRemaining_LowWaterMark = TotalBytesRemaining;
