...
void task_state_est(void *argument) {
  ...
  while (1) {
    ...
    if ((global_flight_state.flight_state == IDLE) && (global_flight_state.state_changed == 1)) {
      reset_kalman(&filter, average_pressure);
      calibrate_imu(&average_imu, &calibration, &elimination);
    }

    if ((global_flight_state.flight_state == APOGEE) && (global_flight_state.state_changed == 1)) {
    ...
    get_data_float(&state_data, &filter, &calibration, &global_flight_state);
