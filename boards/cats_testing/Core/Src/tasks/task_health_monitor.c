...
void task_health_monitor(void *argument) {
  ...
  while (1) {
    ...
    if (global_flight_state.flight_state == IDLE && global_flight_state.state_changed)
      buzzer_queue_status(CATS_STATUS_CHANGED_READY);
    if (global_flight_state.flight_state == MOVING && global_flight_state.state_changed)
