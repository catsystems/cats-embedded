...
const peripheral_out_fp output_table[NUM_OUTPUT_FUNCTIONS] = {
    no_output_function,        high_current_channel_one,  high_current_channel_two, high_current_channel_three,
    high_current_channel_four, high_current_channel_five, high_current_channel_six, low_level_channel_one,
    low_level_channel_two,     low_level_channel_three,   low_level_channel_four,   servo_channel_one,
    servo_channel_two,         servo_channel_three,       servo_channel_four,       set_recorder_state};
...
bool high_current_channel_one(int16_t state) {
  if (state == 0 || state == 1) {
    HAL_GPIO_WritePin(PYRO1_GPIO_Port, PYRO1_Pin, state);
    return true;
  } else
    return false;
}
...
bool high_current_channel_two(int16_t state) {
  if (state == 0 || state == 1) {
    HAL_GPIO_WritePin(PYRO2_GPIO_Port, PYRO2_Pin, state);
    return true;
  } else
    return false;
}
...
bool high_current_channel_three(int16_t state) {
  if (state == 0 || state == 1) {
    HAL_GPIO_WritePin(PYRO3_GPIO_Port, PYRO3_Pin, state);
    return true;
  } else
    return false;
}
...
bool low_level_channel_one(int16_t state) {
  if (state == 0 || state == 1) {
    HAL_GPIO_WritePin(IO1_GPIO_Port, IO1_Pin, state);
    return true;
  } else
    return false;
}
...
bool low_level_channel_two(int16_t state) {
  if (state == 0 || state == 1) {
    HAL_GPIO_WritePin(IO2_GPIO_Port, IO2_Pin, state);
    return true;
  } else
    return false;
}
...
bool set_recorder_state(int16_t state) {
  ...
  if (rec_status >= REC_OFF && rec_status <= REC_WRITE_TO_FLASH) {
    if (global_recorder_status == REC_OFF && state > REC_OFF) {
