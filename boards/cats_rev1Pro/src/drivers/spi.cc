...
void spi_init(SPI_BUS *bus) {
 ...
 uint8_t spi_transmit_receive(SPI_BUS *bus, uint8_t *tx_buf, uint16_t tx_size, uint8_t *rx_buf,
  ...
  HAL_GPIO_WritePin(bus->cs_port, bus->cs_pin, static_cast<GPIO_PinState>(bus->cs_type));
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi) {
  for (int i = 0; i < instance_count; i++) {
    if (buses[i]->busy) {
      HAL_GPIO_WritePin(buses[i]->cs_port, buses[i]->cs_pin, static_cast<GPIO_PinState>(!buses[i]->cs_type));
      buses[i]->busy = false;
    }
  }
}
