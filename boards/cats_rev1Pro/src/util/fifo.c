...
void fifo_init(fifo_t *fifo, uint8_t *pdata, uint32_t size) {
  fifo->data = pdata;
  fifo->size = size;
  fifo->id = osSemaphoreNew(1, 1, NULL);
  fifo_flush(fifo);
}

void fifo_flush(fifo_t *fifo) {
  if (osSemaphoreAcquire(fifo->id, 0U) == osOK) {
    fifo->tail = 0;
    fifo->head = 0;
    fifo->used = 0;
    osSemaphoreRelease(fifo->id);
  }
}
...
bool fifo_write_bytes(fifo_t *fifo, uint8_t *data, uint32_t count) {
  ...
  if (osSemaphoreAcquire(fifo->id, 0U) == osOK) {
    if ((fifo->size - fifo->used) < count) return false;
