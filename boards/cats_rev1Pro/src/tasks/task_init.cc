...
static void create_event_map() {
  ...
  event_action_map = static_cast<event_action_map_elem_t *>(calloc(NUM_EVENTS, sizeof(event_action_map_elem_t)));
  ...
  for (int ev_idx = 0; ev_idx < NUM_EVENTS; ev_idx++) {
    ...
    if (nr_actions > 0) {
      ...
      event_action_map[ev_idx].action_list =
          static_cast<peripheral_act_t *>(calloc(nr_actions, sizeof(peripheral_act_t)));
      // Loop over all actions
      for (uint16_t act_idx = 0; act_idx < nr_actions; act_idx++) {
        if (cc_get_action(static_cast<cats_event_e>(ev_idx), act_idx, &action) == true) {
