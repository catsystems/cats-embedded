...
static void cli_cmd_ls(const char *cmd_name, char *args) {
  if (args == NULL) {
    ...
    char *full_path = static_cast<char *>(malloc(full_path_len + 1));
...
static void cli_cmd_cd(const char *cmd_name, char *args) {
  ...
  if (args == NULL || strcmp(args, "/") == 0) {
    ...
    if (args[0] == '/') {
      ...
      char *tmp_path = static_cast<char *>(malloc(full_path_len + 1));
...
static void print_action_config() {
  ...
  for (int i = 0; i < NUM_EVENTS; i++) {
    int nr_actions = cc_get_num_actions(static_cast<cats_event_e>(i));
    if (nr_actions > 0) {
