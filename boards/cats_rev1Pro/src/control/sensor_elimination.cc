...
void check_sensors(sensor_elimination_t *elimination) {
  ...
  for (uint8_t i = 0; i < NUM_ACCELEROMETER; i++) {
    status = static_cast<cats_error_e>(check_sensor_bounds(elimination, i, &acc_info[NUM_IMU + i]) |
  ...
  for (uint8_t i = 0; i < NUM_IMU; i++) {
    status = static_cast<cats_error_e>(check_sensor_bounds(elimination, i, &acc_info[i]) |
  ...
  for (uint8_t i = 0; i < NUM_BARO; i++) {
    status = static_cast<cats_error_e>(check_sensor_bounds(elimination, i, &baro_info[i]) |
  ...
  for (uint8_t i = 0; i < NUM_MAGNETO; i++) {
    status = static_cast<cats_error_e>(check_sensor_bounds(elimination, i, &mag_info[i]) |
