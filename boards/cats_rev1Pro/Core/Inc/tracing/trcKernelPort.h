...
#define xQueueGiveFromISR__pcHead(__a, __b)                                                   \
  ...
  BaseType_t xQueueGiveFromISR(__a, __b) {                                                    \
    return MyWrapper_xQueueGiveFromISR(xQueue, pxHigherPriorityTaskWoken, queueSEND_TO_BACK); \
  }                                                                                           \
...
#define traceSTREAM_BUFFER_RESET(xStreamBuffer)                                                                       \
  trcKERNEL_HOOKS_KERNEL_SERVICE(                                                                                     \
      prvGetStreamBufferType(xStreamBuffer) > 0 ? TRACE_MESSAGEBUFFER_RESET : TRACE_STREAMBUFFER_RESET, STREAMBUFFER, \
      xStreamBuffer);                                                                                                 \
...
#define traceQUEUE_SEND(pxQueue)                                                                                      \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                    \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) {                  \
        case queueQUEUE_TYPE_BASE:                                                                                    \
...
#define traceQUEUE_SEND_FAILED(pxQueue)                                                                             \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                  \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) {                \
        case queueQUEUE_TYPE_BASE:                                                                                  \
          prvTraceStoreEvent2(                                                                                      \
              xCopyPosition == queueSEND_TO_BACK ? PSF_EVENT_QUEUE_SEND_FAILED : PSF_EVENT_QUEUE_SEND_FRONT_FAILED, \
...
#define traceBLOCKING_ON_QUEUE_SEND(pxQueue)                                                                      \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) {              \
        case queueQUEUE_TYPE_BASE:                                                                                \
          prvTraceStoreEvent2(                                                                                    \
              xCopyPosition == queueSEND_TO_BACK ? PSF_EVENT_QUEUE_SEND_BLOCK : PSF_EVENT_QUEUE_SEND_FRONT_BLOCK, \
              (uint32_t)pxQueue, pxQueue->uxMessagesWaiting);                                                     \
          break;                                                                                                  \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                                                    \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                                                  \
...
#define traceQUEUE_SEND_FROM_ISR(pxQueue)                                                                           \
  if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) {                  \
      case queueQUEUE_TYPE_BASE:                                                                                    \
        prvTraceStoreEvent2(                                                                                        \
            xCopyPosition == queueSEND_TO_BACK ? PSF_EVENT_QUEUE_SEND_FROMISR : PSF_EVENT_QUEUE_SEND_FRONT_FROMISR, \
            (uint32_t)pxQueue, pxQueue->uxMessagesWaiting + 1);                                                     \
        break;                                                                                                      \
      case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                                                        \
      case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                                                      \
...
#define traceQUEUE_RECEIVE(pxQueue)                                                                  \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                   \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) { \
        case queueQUEUE_TYPE_BASE:                                                                   \
          if (isQueueReceiveHookActuallyPeek)                                                        \
            prvTraceStoreEvent3(PSF_EVENT_QUEUE_PEEK, (uint32_t)pxQueue, xTicksToWait,               \
...
#define traceQUEUE_RECEIVE_FAILED(pxQueue)                                                                        \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) {              \
        case queueQUEUE_TYPE_BASE:                                                                                \
          prvTraceStoreEvent3(                                                                                    \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_QUEUE_PEEK_FAILED : PSF_EVENT_QUEUE_RECEIVE_FAILED,      \
              (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting);                                       \
          break;                                                                                                  \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                                                    \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                                                  \
          prvTraceStoreEvent3(                                                                                    \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_SEMAPHORE_PEEK_FAILED : PSF_EVENT_SEMAPHORE_TAKE_FAILED, \
              (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting);                                       \
          break;                                                                                                  \
        case queueQUEUE_TYPE_MUTEX:                                                                               \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                                                     \
          prvTraceStoreEvent2(                                                                                    \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_MUTEX_PEEK_FAILED : PSF_EVENT_MUTEX_TAKE_FAILED,         \
...
#define traceBLOCKING_ON_QUEUE_RECEIVE(pxQueue)                                                                 \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                              \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) {            \
        case queueQUEUE_TYPE_BASE:                                                                              \
          prvTraceStoreEvent3(                                                                                  \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_QUEUE_PEEK_BLOCK : PSF_EVENT_QUEUE_RECEIVE_BLOCK,      \
              (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting);                                     \
          break;                                                                                                \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                                                  \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                                                \
          prvTraceStoreEvent3(                                                                                  \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_SEMAPHORE_PEEK_BLOCK : PSF_EVENT_SEMAPHORE_TAKE_BLOCK, \
              (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting);                                     \
          break;                                                                                                \
        case queueQUEUE_TYPE_MUTEX:                                                                             \
        case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                                                   \
          prvTraceStoreEvent2(                                                                                  \
              isQueueReceiveHookActuallyPeek ? PSF_EVENT_MUTEX_PEEK_BLOCK : PSF_EVENT_MUTEX_TAKE_BLOCK,         \
              (uint32_t)pxQueue, xTicksToWait);                                                                 \
          break;                                                                                                \
      }
...
#define traceQUEUE_PEEK_FAILED(pxQueue)                                                              \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                   \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) { \
        case queueQUEUE_TYPE_BASE:                                                                   \
          prvTraceStoreEvent3(PSF_EVENT_QUEUE_PEEK_FAILED, (uint32_t)pxQueue, xTicksToWait,          \
...
#define traceBLOCKING_ON_QUEUE_PEEK(pxQueue)                                                         \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                   \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) switch (pxQueue->ucQueueType) { \
        case queueQUEUE_TYPE_BASE:                                                                   \
          prvTraceStoreEvent3(PSF_EVENT_QUEUE_PEEK_BLOCK, (uint32_t)pxQueue, xTicksToWait,           \
                              pxQueue->uxMessagesWaiting);                                           \
          break;                                                                                     \
        case queueQUEUE_TYPE_BINARY_SEMAPHORE:                                                       \
        case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                                                     \
...
#define traceTIMER_COMMAND_SEND_8_0_CASES(tmr)                                                                      \
  ...
  case tmrCOMMAND_CHANGE_PERIOD_FROM_ISR:                                                                           \
    prvTraceStoreEvent2(                                                                                            \
        (xReturn == pdPASS) ? PSF_EVENT_TIMER_CHANGEPERIOD_FROMISR : PSF_EVENT_TIMER_CHANGEPERIOD_FROMISR_FAILED,   \
        (uint32_t)tmr, xOptionalValue);                                                                             \
    break;
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_8_X_X */
#define traceTIMER_COMMAND_SEND_8_0_CASES(tmr)
...
#define traceTIMER_COMMAND_SEND(tmr, xCommandID, xOptionalValue, xReturn)                                              \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                     \
    if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask) switch (xCommandID) {                                 \
        case tmrCOMMAND_START:                                                                                         \
          prvTraceStoreEvent1((xReturn == pdPASS) ? PSF_EVENT_TIMER_START : PSF_EVENT_TIMER_START_FAILED,              \
                              (uint32_t)tmr);                                                                          \
          break;                                                                                                       \
        case tmrCOMMAND_STOP:                                                                                          \
          prvTraceStoreEvent1((xReturn == pdPASS) ? PSF_EVENT_TIMER_STOP : PSF_EVENT_TIMER_STOP_FAILED,                \
                              (uint32_t)tmr);                                                                          \
          break;                                                                                                       \
        case tmrCOMMAND_CHANGE_PERIOD:                                                                                 \
          prvTraceStoreEvent2(                                                                                         \
              (xReturn == pdPASS) ? PSF_EVENT_TIMER_CHANGEPERIOD : PSF_EVENT_TIMER_CHANGEPERIOD_FAILED, (uint32_t)tmr, \
              xOptionalValue);                                                                                         \
          break;                                                                                                       \
...
#define tracePEND_FUNC_CALL_FROM_ISR(func, arg1, arg2, ret)                                                 \
  prvTraceStoreEvent1(                                                                                      \
      (ret == pdPASS) ? PSF_EVENT_TIMER_PENDFUNCCALL_FROMISR : PSF_EVENT_TIMER_PENDFUNCCALL_FROMISR_FAILED, \
      (uint32_t)func);
...
#define traceEVENT_GROUP_WAIT_BITS_END(eg, bitsToWaitFor, wasTimeout)                                                  \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                     \
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)                                                   \
      prvTraceStoreEvent2(                                                                                             \
          (wasTimeout != pdTRUE) ? PSF_EVENT_EVENTGROUP_WAITBITS : PSF_EVENT_EVENTGROUP_WAITBITS_FAILED, (uint32_t)eg, \
          bitsToWaitFor);
...
#define traceSTREAM_BUFFER_CREATE_FAILED(xIsMessageBuffer)                                                       \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                               \
    prvTraceStoreEvent2(                                                                                         \
        xIsMessageBuffer == 1 ? PSF_EVENT_MESSAGEBUFFER_CREATE_FAILED : PSF_EVENT_STREAMBUFFER_CREATE_FAILED, 0, \
        xBufferSizeBytes);
...
#define traceSTREAM_BUFFER_DELETE(xStreamBuffer)                                                                      \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                    \
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, pxStreamBuffer) & CurrentFilterMask)                                    \
      prvTraceStoreEvent2(                                                                                            \
          prvGetStreamBufferType(xStreamBuffer) > 0 ? PSF_EVENT_MESSAGEBUFFER_DELETE : PSF_EVENT_STREAMBUFFER_DELETE, \
...
#define traceSTREAM_BUFFER_RESET(xStreamBuffer)                                                                     \
  if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & CurrentFilterMask)                                  \
    if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, xStreamBuffer) & CurrentFilterMask)                                   \
      prvTraceStoreEvent2(                                                                                          \
          prvGetStreamBufferType(xStreamBuffer) > 0 ? PSF_EVENT_MESSAGEBUFFER_RESET : PSF_EVENT_STREAMBUFFER_RESET, \
