...
#define TXRXBuffSize 20
  ...
  void Config(uint8_t bw, uint8_t sf, uint8_t cr, uint32_t freq, uint8_t PreambleLength,
              bool InvertIQ, uint8_t PayloadLength, uint32_t interval, uint32_t flrcSyncWord = 0,
              uint16_t flrcCrcSeed = 0, uint8_t flrc = 0);
 ...
 private:
  ...
  void SetPacketParamsFLRC(uint8_t HeaderType, uint8_t crc, uint8_t PreambleLength,
                           uint8_t PayloadLength, uint32_t syncWord, uint16_t crcSeed);

  void SetDioIrqParams(uint16_t irqMask, uint16_t dio1Mask = SX1280_IRQ_RADIO_NONE,
